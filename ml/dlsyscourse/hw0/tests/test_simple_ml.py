...
def test_nn_single_layer_epoch():
    np.random.seed(0)
    X = np.random.randn(50,5).astype(np.float32)
    y = np.random.randint(3, size=(50,)).astype(np.uint8)  # batch_size 50
    W = np.random.randn(5, 3).astype(np.float32) / np.sqrt(10)  # 5 * 3 weights
    dW = nd.Gradient(lambda W_ : 
        softmax_loss(X @ W_.reshape(5, 3), y))(W)


    W_0 = W.copy()
    nn_single_layer_epoch(X, y, W, lr=1.0, batch=50)

    print("test:")
    print(f"dW.reshape(5,3): {dW.reshape(5,3)}")
    print(f"W_0-W: {W_0-W}")
    np.testing.assert_allclose(dW.reshape(5,3), W_0-W, rtol=1e-4, atol=1e-4)
...
def test_nn_epoch():
    ...
    print(f"X@W1_.shape = {(X@W1).shape}")
    print(f"(np.maximum(X@W1.reshape(5,10),0)@W2).shape = {(np.maximum(X@W1.reshape(5,10),0)@W2).shape}")
    ...
    print(f"dW1: {dW1}")
    print(f"dW2: {dW2}")
    print(f"prediction: {np.maximum(X@W1.reshape(5,10),0)@W2}")
